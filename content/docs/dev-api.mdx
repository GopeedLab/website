...
description: Use the Gopeed HTTP API and official SDKs to create and manage download tasks.
---
...
<Callout type="info">
  You can set any `port`, but be careful not to use ports that are already in
  use or reserved by the system, as this may cause access failures.
</Callout>
...
More usages, source code and package updates are available in the [official Gopeed JS repository](https://github.com/GopeedLab/gopeed-js).
...
Of course, you can also directly call the API through HTTP requests. For details, please refer to the [API documentation](/docs/openapi).
