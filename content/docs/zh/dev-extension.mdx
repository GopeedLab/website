---
title: 扩展开发
description: 学习如何开发 Gopeed JavaScript 扩展、进行本地调试，并理解扩展清单和运行机制。
---
...
- `scripts`：`敲重点！`这里是 Gopeed 扩展激活事件的配置。
  ...
  示例项目中配置的`onResolve`事件会在解析任务时触发，通过`match.urls`字段来匹配任务创建的 url，如果匹配成功，则会运行`entry`字段指定的脚本文件。
...
另外需要注意一点的是，goja 原生支持大部分`es6+`的语法，但是极少部分语法是不支持的，比如`async generator`，但是没关系，通过脚手架创建的项目中已经配置好了`babel`，你可以愉快的使用最新的 es 语法，脚本最终会被编译成`es5`语法。
...
简而言之就是需要在`onResolve`回调函数中，根据`ctx.req`里的请求信息，解析出需要下载的文件列表赋值给`ctx.res`即可。

> 关于`ctx`相关类型定义与扩展源码，可以参考[官方 Gopeed JS 仓库](https://github.com/GopeedLab/gopeed-js)。
...
日志文件在 Gopeed 安装目录里的`logs`目录下，文件名称为`extension.log`，可以通过`tail -f extension.log`命令实时查看日志。
...
发布到远端仓库之后，就可以在 Gopeed 中安装了，打开扩展页面，输入扩展的 HTTP 协议的`git clone`地址(可以省略掉后面的`.git`后缀)，点击`安装`按钮即可进行安装。

> 注：如果扩展目录是一个子目录，需要在地址后面加上`#`，再加上子目录名称，例如`https://github.com/xxx/gopeed-extension-demo#path`。

## 官方示例

官方提供了两个具有代表性的示例扩展供参考，分别是：
