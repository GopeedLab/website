---
title: 介绍
description: 了解 Gopeed 是什么、支持哪些下载协议，以及它为什么适合作为跨平台下载管理器。
---

Gopeed（全称 Go Speed），直译过来中文名叫做`够快下载器`（不是狗屁下载器！），是一款由 `Golang` + `Flutter` 开发的高速下载器，支持 `HTTP`、`BitTorrent`、`Magnet` 和 `ed2k` 协议下载，并且支持所有平台使用，支持的平台有：
...
作为一个现代化的下载器，Gopeed 有着非常多的特性，下面来简单的介绍一下。
